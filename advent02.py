idx = """ubkfmdjxyzlbgkrotcepvswaqx
...
uikfmdjxuzlbgnrotcepvsvaqc""".split('\n')

# Part One
twos = 0
threes = 0
for ids in idx:
	if any(ids.count(l)==2 for l in set(ids)):
		twos += 1
	if any(ids.count(l)==3 for l in set(ids)):
		threes += 1
print twos * threes
...
for i, ids1 in enumerate(idx):
	for ids2 in idx[i+1:]:
		if sum([a!=b for a, b in zip(ids1, ids2)]) == 1:
			print ''.join([a for i, a in enumerate(ids1) if a==ids2[i] ])
			break