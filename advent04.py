entries = """[1518-03-11 00:45] wakes up
...
[1518-08-01 00:40] falls asleep""".split('\n')
...
for entry in entries:
	...
	if 'wakes' in entry:
		if current not in sleeps.keys():
			sleeps[current] = [0] * 60
		for minute in xrange(start, int(entry[15:17])):
			sleeps[current][minute] += 1

guard = max(sleeps.iterkeys(), key=(lambda key: sum(sleeps[key])))
minute = sleeps[guard].index(max(sleeps[guard]))
print "The guard that sleeps the most is %d" % guard
print "He sleeps the most at minute %d" % minute
print "The product is %d" % (guard * minute)

# Part Two
current = (0, 0, 0)
for guard in sleeps.iterkeys():
	occurence = max(sleeps[guard])
	if occurence > current[2]:
		current = (guard, sleeps[guard].index(occurence), occurence)
print "The most alseep minute by a guard is minute %d by guard %d" % (current[0], current[1])
print "The product is %d" % (current[0] * current[1])